$(document).ready(function() {
    var table = $('#datable').DataTable({
        ...
        autoWidth: false,
        ajax: {
            url: "/candidate/training/get-data-trainings",
            dataSrc: 'data',
            type: "GET"
        },
    ...
    let buttons = `
        <div class="dt-buttons flex-wrap">
            <button type="button" class="btn btn-success" id="button-register" data-toggle="modal"><i class="fas fa-folder-open"></i> Nuevo</button>
    ...
    $('#button-register').on('click', function(e) {
        ...
        $('#modalRegisterTraining').modal('show')
    ...
    $('#formTraining').on('submit', function(e) {
        ...
        if (form[0].checkValidity()) {
            ...
            data.append('certificate', $("#formTraining input[name=certificate]")[0].files[0])
            $.ajax({
                url: '/candidate/training/register-training',
                type: 'POST',
                data: data,
                processData: false,
                contentType: false,
                success: function(data) {
                    if (data.success) {
                        $('#modalRegisterTraining').modal('hide')
                        Swal.close();
                        form[0].reset()
    ...
    $('#button-edit').on('click', function(e) {
        ...
        if (data !== undefined) {
            console.log(data)
            $('#formeEditTraining input[name="title"]').val(data.title)
            $('#formeEditTraining input[name="institution"]').val(data.institution)
            $('#formeEditTraining input[name="date_emition"]').val(data.date_emition)
            $('#file_path_certificate').prop('href', `/candidate/training/view-document?id=${data.certificate_file_path}`)
    ...
    $('#formeEditTraining').on('submit', function(e) {
        e.preventDefault()
        e.stopPropagation()
        var form = $(this)
        if (form[0].checkValidity()) {
            ...
            data.append('certificate', $("#formeEditTraining input[name=certificate]")[0].files[0])
